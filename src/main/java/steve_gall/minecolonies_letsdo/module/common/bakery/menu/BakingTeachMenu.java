package steve_gall.minecolonies_letsdo.module.common.bakery.menu;
...
public class BakingTeachMenu extends TeachRecipeMenu<RecipeHolder<BakingStationRecipe>, RecipeInput>
{
	...
	public BakingTeachMenu(int windowId, Inventory inventory, RegistryFriendlyByteBuf buffer)
	{
	...
	@Override
	protected IMenuRecipeValidator<RecipeHolder<BakingStationRecipe>, RecipeInput> createRecipeValidator()
	{
		return new MenuRecipeValidatorRecipe<>(this.inventory.player.level())
		{
			...
			@Override
			public RecipeInput getInput(Container container, RecipeHolder<BakingStationRecipe> recipe)
			{
				return new RecipeWrapper(new InvWrapper(new CompoundContainer(new SimpleContainer(1), container)));
			}

			@Override
			protected boolean test(RecipeHolder<BakingStationRecipe> recipe, Container container, ServerPlayer player)
			{
	...
	@Override
	protected void setContainerByTransfer(@NotNull HolderLookup.Provider provider, @NotNull RecipeHolder<BakingStationRecipe> recipe, @NotNull CompoundTag payload)
	{
